...
def required_mw(chord_km, detector_kt=10, slot_s=1.0):
    """Peak on-slot neutrino-carried MW at the specified slot duration."""
    return -math.log(2*TARGET_BER)*neutrino_energy_per_selected_event(chord_km,detector_kt)/slot_s/1e6

def detector_kt(chord_km, neutrino_on_mw, slot_s=1.0):
    return required_mw(chord_km,10,slot_s)*10/neutrino_on_mw
...
def main():
    ...
    with (OUT/'power_detector_tradeoff.csv').open('w',newline='') as f:
        w=csv.writer(f);w.writerow(['chord_km','beam_half_angle_mrad','neutrino_energy_GeV','selected_efficiency','target_raw_BER','slot_duration_s','peak_neutrino_carried_on_slot_MW','required_detector_kt'])
    ...
    with (OUT/'latency_example.csv').open('w',newline='') as f:
        w=csv.writer(f);w.writerow(['chord_km','detector_kt','peak_neutrino_carried_on_slot_MW','idealized_required_on_slot_s','neutrino_propagation_ms','ideal_shortest_surface_fiber_ms','ideal_fiber_advantage_ms'])
        l=5000
    ...
    print('5000 km 10 kt neutrino MW for 1 ms slot:',required_mw(5000,10,.001))
